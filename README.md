...
|      Element      | Description                                                                                                                                                                                                                                                            |
|:---------------------:|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|         data/         | This folder containes all the reports that were generated by Spark. Each sub-folder has the ``data.csv`` file with the table and its schema in the ``schema.json`` file.                                                          |
| data/report_names.txt | This is a text file that contains all the names of the reports included in the data folder. This list will be used in the pipeline to read each report and create its corresponding table in BigQuery.                                                           |
|       src/       | This folder is where the file [pipeline.py](.src/pipeline.py) is stored. The python script will generate the jobs in Dataflow to read the data from Cloud Storage and load it into BigQuery. |
